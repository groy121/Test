...
import com.cbp.rest.database.dao.IUserDAO;
import com.cbp.rest.database.dao.UserDAO;
import com.cbp.rest.exception.UserNotFoundException;
import com.cbp.rest.model.User;
import com.cbp.rest.model.UserSecurity;
import com.cbp.rest.restapi.ResponseBuilder;
import com.cbp.rest.security.TokenSecurity;
...
public class AuthenticationFilter implements javax.ws.rs.container.ContainerRequestFilter
{
    ...
    public void filter( ContainerRequestContext requestContext )
    {
        ...
        if( !method.isAnnotationPresent( PermitAll.class ) )
        {
            ...
            IUserDAO userDao = new UserDAO(new DBConnection());
            User user = null;
            try {
            	user = userDao.getUser( id );
            }
            catch ( UserNotFoundException e ) {
            	System.out.println("Token missmatch!");
                requestContext.abortWith( 
                    	ResponseBuilder.createResponse( Response.Status.UNAUTHORIZED, ACCESS_DENIED )
                );
            	return;
            ...
            if( method.isAnnotationPresent( RolesAllowed.class ) )
            {
                ...
                {
                	System.out.println("User does not have the rights to acces this resource!");
                    requestContext.abortWith( 
                        	ResponseBuilder.createResponse( Response.Status.UNAUTHORIZED, ACCESS_DENIED )
                    );
                    return;
